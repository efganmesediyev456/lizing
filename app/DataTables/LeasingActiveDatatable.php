...
class LeasingActiveDatatable extends DataTable
{
    protected $tableId;
    ...
    public function query(Leasing $model)
    {
        ...
        $query = $model->newQuery()->with([
            ...
            'vehicle',
            'leasingStatus'
        ])->where(function ($q) use ($passiveStatus) {
            $q->whereNot('leasing_status_id', $passiveStatus->id)
                ->whereNot('leasing_status_id', 3)
                ->orWhereNull('leasing_status_id');
        });
        ...
        if (request()->filled('status_id')) {
            $query = $query->where('leasing_status_id', request()->status_id);
        }
        ...
        if (request()->filled('table_id_number')) {
            $query = $query->where('tableId', request()->table_id_number);
        }
        if (request()->filled('state_registration_number')) {
            $query = $query->where('vehicle_id', request()->state_registration_number);
        }
        if (request()->filled('has_advertisement')) {
            $query = $query->where('has_advertisement', request()->has_advertisement);
        }
