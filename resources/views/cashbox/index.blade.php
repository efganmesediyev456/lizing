        ...
        <div class="cashbox-body technical-body">
            ...
            <div class="d-flex">
                <div class="total-payment">
                    <p>Xərc: </p>
                    <span id="totalExpenseHolder">{{ $totalExpense }} azn</span>
                </div>
                <div class="total-payment">
                    <p>Gəlir: </p>
                    <span id="totalIncomeHolder">{{ $totalInsurance }} azn</span>
                </div>
                <div class="total-payment">
                    <p>Ümumi kassa: </p>
                    <span id="totalPriceHolder">{{ $totalCashboxPrice }} azn</span>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    ...
    <script>
        $(function() {
            let tableId = '{{ $tableId ?? 'cities-table' }}';
            ...
            //     $('#totalExpenseHolder').text(json.totalExpense + ' AZN');
            //     $('#totalIncomeHolder').text(json.totalIncome + ' AZN');
            // });
            ...
            function updatePagination() {
                ...
                function createPageItem(i, isActive = false) {
                    let activeClass = isActive ? 'active' : '';
                    return $('<a href="#" class="pagination-item ' + activeClass + '">' + i + '</a>');
                }

                function createDots() {
                    return $('<span class="pagination-item dots">...</span>');
                }
                ...
                if (totalPages <= 10) {
                    ...
                    pagesToShow.push(1);
                    if (currentPage > 4) pagesToShow.push('dots-start');
                    ...
                    if (currentPage < totalPages - 3) pagesToShow.push('dots-end');
                    pagesToShow.push(totalPages);
                }

                pagesToShow.forEach(page => {
                    if (page === 'dots-start' || page === 'dots-end') {
                        createDots().insertBefore(pagination.find('.next'));
                    } else {
                        createPageItem(page, page === currentPage).insertBefore(pagination.find('.next'));
                    }
                });
            ...
            $(".generalSearch [name='date']").on("change", function() {
                var value = $(this).val();
