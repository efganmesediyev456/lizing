 ...
 <div class="drivers-container">
            <div class="drivers-container-head">
                ...
                <div class="head-buttons">
                    <a href="{{ route('drivers.export') }}" class="export_excel">
                    ...
                    <button class="addNewDriver addNewUsers addItem" type="button">
            ...
            <div class="drivers-tabs">
                    <button class="driver-tab active" data-id="all">Hamısı</button>
                    @foreach($driverStatuses as $status)
                    <button class="driver-tab" data-id="{{ $status->id }}">{{ $status->status }}</button>
                    @endforeach
            </div>
...
<script>
    $(function() {
        let tableId = '{{ $tableId ?? "drivers-table" }}';
        ...
        $("body").on('click', '.addNotification', function() {
            ...
            var id = $(this).attr('data-id')
            $.ajax({
                url: "{{ route('notification.form') }}" + "/" + id,
                type: "post",
                data: {
                    _token: '{{csrf_token()}}'
                },
        ...
        $('body').on('submit', '.saveForm', function(e) {
            e.preventDefault();
            var form = this;
            document.querySelectorAll(".editable-label").forEach(function(span) {
                let hidden = span.nextElementSibling; 
                if (hidden && hidden.type === "hidden") {
                    hidden.value = span.innerText.trim(); 
                }
            });
        ...
        initFileInputUploadStyle('.drivers_license_front');
        initFileInputUploadStyle('.drivers_license_back');
        ...
        $(".drivers-tabs .driver-tab").on("click", function(e){
            var statusId = $(this).attr('data-id');
            $(".drivers-tabs .driver-tab").removeClass('active')
            $(this).addClass("active")

            let baseUrl = "/drivers";
            let newUrl = baseUrl + '?status_id=' + statusId;
