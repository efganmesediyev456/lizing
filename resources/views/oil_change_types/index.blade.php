...
@section('content')
    <div class="oilChangeType-container">
        <div class="oilChangeType-container-head">
            <h1>Yağın dəyişilmə növü</h1>
            <div class="head-buttons">
                ...
                <button class="addNewOilChangeType addNewUsers addItem" type="button">
                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                        ...
                        <path
                            ...
                            fill="white" />
                                       ...
                                       Yeni Yağın dəyişilmə növü Əlavə et
                </button>
            </div>
        </div>
        <div class="oilChangeType-body">
            <div class="oilChangeType-body-head">
...
@push('js')
    ...
    <script>
        $(function() {
            let tableId = '{{ $tableId ?? 'oil_change_types-table' }}';
            ...
            $(".addItem").click(function() {
                ...
                $(".saveForm").remove()
                $.ajax({
                    url: "{{ route('oil_change_types.form') }}",
                    type: "post",
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
            ...
            $("body").on('click', '.editItem', function() {
                ...
                $.ajax({
                    url: "{{ route('oil_change_types.form') }}" + "/" + id,
                    type: "post",
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    ...
                    error: function(xhr) {
                        for (let a in xhr.responseJSON.errors) {
                            for (let b in xhr.responseJSON.errors[a]) {
                                $(form).find("[name='" + a + "']").parents('.form-item').append(
                                    '<p class="formError" style="color:red">' + xhr
