...
@section('content')
    ...
    <div class="leasing-container">
        <div class="leasing-container-head">
            ...
            <div class="head-buttons">
                ...
                <button class="addNewLeasing addNewUsers addItem" type="button">
        ...
        <div class="drivers-tabs">
            <a href="{{ route('leasing.index') }}" class="driver-tab active" data-id="all">Hamısı</a>
            <a href="{{ route('leasing.active') }}" class="driver-tab" data-id="">Aktiv borclular</a>
            <a href="{{ route('leasing.passiv') }}" class="driver-tab" data-id="">Passiv borclular</a>
        </div>

        <div class="leasing-body">
            <div class="leasing-body-head">
    ...
    <div class="add-driver-container" id="createPayment">
        <div class="add-driver">
            ...
            <form action="{{ route('leasing.payment') }}" id="submitPaymentForm">
                <div class="form-items">

                    <div class="form-item">
                        <label for="">Ödəniş növü</label>
                        <select name="payment_status" id="">
                            <option value="monthly_payment">Aylıq ödəniş</option>
                            <option value="daily_payment">Günlük ödəniş</option>
                            <option value="deposit_price">İlkin depozit ödəniş</option>
                            <option value="deposit_debt">depozit ödəniş</option>
                        </select>
                    </div>


                    <input type="hidden" name="leasing_id" value="">

                    <div class="form-item">
                        <label for="">Ödəniləcək məbləğ</label>
                        <div class="form-input">
                            <input type="number" any=".01" name="price">
                        </div>
                    </div>
                ...
                <button class="submit" type="submit">Ödənişi əlavə et</button>
            </form>
        </div>
    </div>
...
@push('js')
    ...
    <script>
        $(function() {
            ...
            $(document).on('change', 'select[name="leasing_status_id"]', function() {
                let selectedOption = $(this).find('option:selected');
                let isPassive = selectedOption.data('passive');

                if (isPassive == 1) {
                    let modalHtml = `
                    <div class="add-driver-container" id="passiveReasonContainer" style="display:flex;">
                        <div class="add-driver">
                            <h2>Passiv Səbəb</h2>
                            <button class="closeAddDriver" type="button">
                                <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <form id="passiveReasonForm">
                                
                                    <div class="" style="width:100%">
                                        <label for="">Passiv Səbəbi</label>
                                        <textarea class="passive_reason" name="passive_reason"  required></textarea>
                                    </div>
                               
                                <button class="submit" type="submit">Təsdiq et</button>
                            </form>
                        </div>
                    </div>
                `;
                    ...
                    $(".saveForm").find("[name='passive_reason']").remove()
                }
            });

            $(document).on('click', '.closeAddDriver', function() {
                $('#passiveReasonContainer').remove();
            });

            $(document).on('submit', '#passiveReasonForm', function(e) {
                e.preventDefault();
                $('#passiveReasonContainer').remove();
            });
            ...
            // <
            // !--tesdiqleme leasing sebebini-- >

            $(document).on('submit', '#passiveReasonForm', function(e) {
                e.preventDefault();

                let passiveReason = $(this).find('.passive_reason').val();

                $('.saveForm').append(`
                <textarea name="passive_reason" style="display:none;">${passiveReason}</textarea>
            `);
            ...
            $("#submitPaymentForm").on("submit", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $(".success-modal-container h2").text("Uğurlu əməliyyat!");
                        $(".success-modal-container p").text("Ödəniş uğurla əlavə olundu.");
